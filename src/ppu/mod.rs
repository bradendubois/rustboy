...
use crate::traits::{Byte, MemoryMap, RunComponent};
...
pub struct PPU {
    ...
    clock: i64,     // Behaves as a counter of how many cycles / ticks have occurred, used to determine
 ...
 */
    ...
    fn background_pixels(&mut self) -> Vec<(Point, Color)> {
        ...
        for i in 0..WIDTH {
            ...
            let x = (((self.scx as usize) + i) % WIDTH) as u8;

            let tile_base = self.lcdc.bg_tile_map_display_select().0;
            let object_number = self.read(tile_base + (y_adjustment + (x / 8) as u16));

            let tile = self.get_object(object_number);

            let offset = x % 8;

            let b1 = if (tile[(offset * 2) as usize]) & (1 << (7 - offset)) != 0 { 1 } else { 0 };
            let b0 = if (tile[((offset * 2) + 1) as usize]) & (1 << (7 - offset)) != 0 { 1 } else { 0 };
    ...
    fn get_object(&mut self, object_number: u8) -> Vec<u8> {

        let base = self.lcdc.bg_window_tile_data_select().0;

        let mut bytes: Vec<u8> = Vec::new();

        // 8800 Mode
        let address = if base == registers::lcdc::BG_WINDOW_TILE_DATA_SELECT_0.0 {

            (match object_number {
                  0..=127 => 0x9000,
                128..=255 => 0x8800,
                _ => panic!("impossible object number: {}", object_number)
            } + (object_number as u16 * 16)) as u16

        // 8000 Mode
        } else { 0x8000 + ((object_number as u16) * 16)  };

        println!("address: {:#06X}", address);

        // Load bytes
        for i in 0..16 {
            let value = self.read(address + (i as u16));
            bytes.push(value);
        }
...
impl RunComponent for PPU {
    ...
    fn run(&mut self, cycles: u64) {
        ...
        if self.clock >= CYCLES_PER_LINE as i64 {
            self.clock %= CYCLES_PER_LINE as i64;
            ...
            if self.lcds.lyc_interrupt() && self.ly == self.lyc {
                ...
                self.lcds.write(self.lcds.read() | 0x04);
...
impl MemoryMap for PPU {
    ...
    fn write(&mut self, address: u16, value: u8) {
        ...
        if address >= 0x8000 && address <= 0x97FF {
            //println!("ppu write: {:#06X} {:010b}", address, value);
        }
        ...
        match address {
            ...
            0xFF46 => panic!("dma transfer not handled by ppu!"),
